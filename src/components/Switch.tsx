...
const Switch: FC<Props> = ({
  ...
  const trackClasses = `w-12 h-6 ring-[1px] rounded-2xl ${transitionDuration}  cursor-pointer`;
  ...
  return (
    <div
      ...
      className={`switch-track ${trackClasses} ${duration} ${
        enabled ? bgDark : bgLight
      }`}
    >
      <div
        className={`switch-thumb ${thumbClasses} ${
          enabled ? bgLight : `translate-x-full ${bgDark}`
        }`}
      >
        <svg
          className={`switch-icon ${iconClasses} ${
